...
def check_server():
    ...
    cmd = ["ping", SERVER_IP, "-n", "4"]
    response = subprocess.run(cmd, stdout=subprocess.PIPE)
    lines = response.stdout.decode("cp850").splitlines()
    
    # Check if the server is online
    if " Received = 4" in lines[-3].split(sep=",")[1]:
        ...
        # Check the ping
        time_line = lines[-1].split(sep=",")[-1]
        ping = float((time_line.split(",")[-1]).split(sep="=")[-1].removesuffix("ms"))
        if ping > 200:
            logger.warning(f"High ping: {ping} ms")
            send_telegram_message("HIGH Ping")
    else:
        ...
        send_telegram_message("Server DOWN")
